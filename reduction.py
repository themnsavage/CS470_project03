...
def main():
    """
    Description: allow user to reduce/map np problems to each other
    """
    ...
    Data_Extractor().convert_json(
        data=knapsack_data_from_three_sat,
        file_path="data/output_reduction_files/knapsack_data_from_3sat.txt",
    )

    knapsack_data = Data_Extractor(
        file_path="data/input_reduction_files/reduce_knapsack_to_traveling_salesman.txt"
    ).get_data()
